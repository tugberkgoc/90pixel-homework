import React from "react"
import searchStore from '../store/searchStore'
import {MdFavorite, MdFavoriteBorder} from "react-icons/md"

const Card = ({id, title, imageUrl, releaseDate, imdbRating, isAddedToFav}) => (

    <div className="col-md-6">
      <div className="card mb-3">
        <div className="row no-gutters">
          <div className="col-md-4">
            {imageUrl !== '' ? <img src={imageUrl} className="card-img" style={{height: "250px"}} alt={title}/> : <img
                src="data:image/png;base64,"
                style={{height: "250px", width: "180px"}}/>}
          </div>
          <div className="col-md-8">
            <div className="card-body">
              <h5 className="card-title">{title} {isAddedToFav ?
                  <span className="pl-1" onClick={() => isAddedToFav = !isAddedToFav}><MdFavorite/></span> :
                  <span className="pl-1" onClick={() => isAddedToFav = !isAddedToFav}><MdFavoriteBorder/></span>}</h5>
              <p className="card-text">{imdbRating === 'N/A' ? '0.0' : imdbRating}</p>
              <p className="card-text">
                {releaseDate !== 'N/A' && <small className="text-muted">{releaseDate}</small>}
              </p>
              {
                isAddedToFav === false
                    ?
                    <button className="btn btn-primary" onClick={() => {
                      searchStore.addToFav({id, title, imageUrl, releaseDate, imdbRating, isAddedToFav: true})
                    }}>Add to Favorites</button>
                    :
                    <button className="btn btn-primary" onClick={() => {
                      searchStore.removeToFav({id, title, imageUrl, releaseDate, imdbRating, isAddedToFav: false})
                    }}>Remove from Favorites</button>
              }
            </div>
          </div>
        </div>
      </div>
    </div>

)

export default Card
